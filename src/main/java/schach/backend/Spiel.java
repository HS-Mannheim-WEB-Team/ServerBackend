...
public class Spiel {
	...
	public Belegung ziehe(String von, String nach) {
		...
		if (b.isBauerDoppelschritt(von, nach))
			zNeu.setBemerkung(ZugEnum.BauerDoppelschritt);
		if (isEnPassant)
			zNeu.setBemerkung(ZugEnum.EnPassant);
	...
	private String getZugAlsNotation(int nummer) {
		...
		if (g2.size() > g1.size())
			fGeschlagen = g2.get(g2.size() - 1);
		
		if ((zugBemerkung != null) && (ZugEnum.RochadeKurz.equals(zugBemerkung)))
			return "0-0";
		if ((zugBemerkung != null) && (ZugEnum.RochadeLang.equals(zugBemerkung)))
			return "0-0-0";
