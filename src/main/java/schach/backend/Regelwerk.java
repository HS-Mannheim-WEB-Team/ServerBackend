...
public final class Regelwerk {
	...
	public static boolean kannSchlagen(Belegung b, String posAngreifer, String posOpfer) {
		if (b == null)
			return false;
		if ((posAngreifer == null) || (posAngreifer.length() != 2))
			return false;
		if ((posOpfer == null) || (posOpfer.length() != 2))
			return false;
		HashSet<Zug> zuege = getErlaubteZuegeOhneSchach(b, posAngreifer, false);
		if ((zuege == null) || (zuege.size() == 0))
			return false;
		for (Zug zug : zuege) {
			if (posOpfer.equals(zug.getNach()))
				return true;
		}
		return false;
	}
	...
	public static HashSet<Zug> getErlaubteZuege(Belegung b, Figur f) {
		...
		if ((b == null) || (f == null) || (f.isGeschlagen()))
			return zuege;
	...
	private static HashSet<Zug> getErlaubteZuegeOhneSchachBauer(Belegung b, Figur f) {
		...
		String posZiel = null;
		if ((y == 1) || (y == 8))
			return zuege; // Bauer ist am Ende angekommen -> kein Zug moeglich
		int richtung = 1;
		if (f.isSchwarz())
			richtung = -1;
	...
	private static HashSet<Zug> getErlaubteZuegeOhneSchachLaeufer(Belegung b, Figur f) {
		...
		for (i = x + 1, j = y + 1; ((i <= 8) && (j <= 8)); i++, j++) {
			if (!addZug(b, posStart, felder, i, j))
				break;
		}
	...
	private static HashSet<Zug> getErlaubteZuegeOhneSchachTurm(Belegung b, Figur f) {
		...
		int i;
		for (i = y + 1; i <= 8; i++) {
			if (!addZug(b, posStart, felder, x, i))
				break;
		}
		for (i = y - 1; i >= 1; i--) {
			if (!addZug(b, posStart, felder, x, i))
		...
		for (i = x + 1; i <= 8; i++) {
			if (!addZug(b, posStart, felder, i, y))
				break;
		}
		for (i = x - 1; i >= 1; i--) {
			if (!addZug(b, posStart, felder, i, y))
	...
	private static HashSet<Zug> getErlaubteZuegeOhneSchachKoenig(Belegung b, Figur f, boolean inclRochadenCheck) {
		...
		if (((!f.isBereitsBewegt()) && (inclRochadenCheck))) { // ich darf mich nicht bereits bewegt haben
			...
			if (b.getStatus() != null) {
				if (f.isWeiss() && (b.getStatus().equals(SpielEnum.WeissImSchach)))
					binImSchach = true;
				if (f.isSchwarz() && (b.getStatus().equals(SpielEnum.SchwarzImSchach)))
					binImSchach = true;
			}
			// 1. Rochade: lange
	...
	private static boolean isPositionBedrohtFuerRochade(Belegung b, String postion, boolean durchWeiss) {
		if ((postion == null) || (postion.length() != 2))
			return false;
